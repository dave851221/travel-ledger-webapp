...
## 📂 專案主要功能
- **多人分帳**: 整合 `decimal.js` 處理高精度運算與公平餘數分配。
- **即時同步**: 使用 Supabase Real-time 技術，多人記帳全家同步。
- **LINE Bot 自然語言記帳**: 整合 Gemini Pro AI，透過 LINE 傳送文字或語音即可記帳。
- **PWA 支援**: 支援離線查看與安裝至手機桌面（添加到主畫面）。
