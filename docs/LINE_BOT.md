...
- **收據的逐項重新分帳**: 聊天室裡若還有一張未確認／未取消的收據草稿，
  後續的文字訊息會**連同那張收據一起**送給 AI（`getOutstandingPhotoDraft`）。
  所以使用者可以說「A 是我吃的，小明吃 B」，AI 重新閱讀收據品項後重算分攤，
  再送出一張新卡片；光靠文字是分不出各品項金額的。
  重算出的草稿沿用原本的 photo_ids，卡片仍帶收據縮圖；帶圖時自動切換到視覺模型清單。
- **舊草稿會失效**: 送出新的記帳建議卡片前，`supersedePendingDrafts()` 會把先前未處理的
  nonce 寫進 `line_processed_actions`，讓聊天室裡的舊卡片按下去走到「此操作已處理過」，
  不會存入未修正的內容。這是重用既有的防重複點擊鎖，不另設一套機制。
